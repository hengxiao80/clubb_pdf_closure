module pdf_skx
  ...
  elemental subroutine pdf_closure &
    ...
    ! --- Heng Xiao 2021/01/02
    ! Here the threshold value (rt_tol) matters for the
    ! reconstructed variances of moisture (rtp2_clipped).
    ! The original value of 1.0e-4 (kg/kg) is too big.
    ! 1.0e-8 is probably an overkill. But there are still small
    ...
    if (wp2 <= w_tol*w_tol) then
      ...
      rtpthlp_clipped = uf*(rt_1-rtm)*(thl_1-thlm) &
                        + (1.0-uf)*(rt_2-rtm)*(thl_2-thlm) &
                        + rrtthl*(uf*sqrt(var_rt_1*var_thl_1) &
                                  + (1.0-uf)*sqrt(var_rt_2*var_thl_2))
    end if
